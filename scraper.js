...
const CONFIG = {
  ...
  delayBetweenRequests: 4000, // Increased delay
  headless: true,
  maxConcurrentPages: 1, // Reduced to 1 for maximum stability
...
// Robust retry mechanism with resource cleanup
...
// Safe page closing with error handling
async function safeClosePage(page) {
  try {
    ...
    console.error('⚠️ Error saat menutup halaman:', e.message);
  }
}
...
async function scrapeKuramanime() {
  ...
  try {
    ...
    try {
      ...
      console.error('⚠️ Error saat menutup browser:', e.message);
    }
